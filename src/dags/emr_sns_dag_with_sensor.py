...
EMR_STEPS = [
    {
        ...
        "HadoopJarStep": {
            ...
            "Args": ["spark-submit", "--deploy-mode", "cluster", "s3://insightrag-job-config/pyspark_ocr.py"]
        },
    }
]
...
def add_steps_to_emr(**kwargs):
    ...
    response = hook.add_job_flow_steps(job_flow_id=job_flow_id, steps=EMR_STEPS)
    kwargs['ti'].xcom_push(key='step_id', value=response["StepIds"][0])
...
with DAG(
    ...
    wait_for_step_completion = EmrStepSensor(
        ...
        job_flow_id="{{ task_instance.xcom_pull(task_ids='create_emr_cluster', key='job_flow_id') }}",
        step_id="{{ task_instance.xcom_pull(task_ids='add_steps', key='step_id') }}",
        poke_interval=60,
        timeout=7200
    )
    ...
    create_emr_cluster_task >> add_steps_task >> wait_for_step_completion >> [send_sns_task, send_sqs_task]
